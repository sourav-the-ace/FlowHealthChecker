...
export default class FlowHealthChecker extends NavigationMixin(
  LightningElement
) {
  ...
  startPolling() {
    ...
    this.pollTimer = setInterval(async () => {
      try {
        ...
        this.isScanning = false;
        this.errorMessage =
          err.body?.message || err.message || "Polling error.";
      }
    }, POLL_INTERVAL_MS);
  }
  ...
  processScanData(data, scanId) {
    ...
    const errors = results.filter(
      (r) => (r.svfhc__Severity__c || r.Severity__c) === "Error"
    ).length;
    const warnings = results.filter(
      (r) => (r.svfhc__Severity__c || r.Severity__c) === "Warning"
    ).length;
    const total =
      scan.svfhc__Total_Flows_Scanned__c || scan.Total_Flows_Scanned__c || 0;
    const affected = new Set(
      results.map((r) => r.svfhc__Flow_API_Name__c || r.Flow_API_Name__c)
    ).size;
    const scanDateVal = scan.svfhc__Scan_Date__c || scan.Scan_Date__c;
    const persistedScore =
      scan.svfhc__Health_Score__c != null
        ? scan.svfhc__Health_Score__c
        : scan.Health_Score__c;
    const score =
      (persistedScore != null
        ? Math.round(persistedScore)
        : this.computeHealthScore(total, errors, warnings)) + "%";
  ...
  rebuildGroups() {
    let data =
      this.activeFilter === "All"
        ? [...this.allResults]
        : this.allResults.filter(
            (r) => r.svfhc__Severity__c === this.activeFilter
          );
    ...
    map.forEach((violations, flowApiName) => {
      const errs = violations.filter(
        (v) => v.svfhc__Severity__c === "Error"
      ).length;
      const warns = violations.filter(
        (v) => v.svfhc__Severity__c === "Warning"
      ).length;
      ...
      groups.push({
        ...
        healthLabel:
          errs > 0 ? "Critical" : warns > 0 ? "Needs Review" : "Healthy"
      });
    });
  ...
  async openHistoryModal() {
    try {
      ...
      this.historyList = history
        ...
        .map((s) => {
          const totalFlows =
            s.svfhc__Total_Flows_Scanned__c || s.Total_Flows_Scanned__c || 0;
          ...
          const persistedScore =
            s.svfhc__Health_Score__c != null
              ? s.svfhc__Health_Score__c
              : s.Health_Score__c;
          const score =
            persistedScore != null
              ? Math.round(persistedScore)
              : this.computeHealthScore(totalFlows, errors, warnings);
          return {
            id: s.Id,
  ...
  computeHealthScore(total, errors, warnings) {
    ...
    const errPoints = Math.min(
      (Number(errors) || 0) * (100 / totalFlowsNum) * 0.6,
      100
    );
    const warnPoints = Math.min(
      (Number(warnings) || 0) * (100 / totalFlowsNum) * 0.2,
      100
    );
  ...
  badgeClass(s) {
    return s === "Error"
      ? "fhc-badge fhc-badge-err"
      : s === "Warning"
        ? "fhc-badge fhc-badge-warn"
  ...
  get filterAllClass() {
    return this.activeFilter === "All"
      ? "fhc-filter-btn active"
      : "fhc-filter-btn";
  }
  get filterErrorClass() {
    return this.activeFilter === "Error"
      ? "fhc-filter-btn active"
      : "fhc-filter-btn";
  }
  get filterWarningClass() {
    return this.activeFilter === "Warning"
      ? "fhc-filter-btn active"
      : "fhc-filter-btn";
  }
  get filterInfoClass() {
    return this.activeFilter === "Info"
      ? "fhc-filter-btn active"
      : "fhc-filter-btn";
  }
  // ── ARIA getters ─────────────────────────────────────────────
